...
const handleScroll = () => {
 ...
 return (
  <div className={`min-h-screen ${theme === 'dark' ? 'dark' : ''} ${theme === 'dark' ? 'bg-gray-700' : 'bg-gray-100'}`}>
    <nav className={`p-5 fixed top-0 w-full z-10 transition duration-500 ease-in-out ${isNavbarTransparent ? 'bg-gray-800 dark:bg-gray-900' : 'bg-transparent'}`}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
        {cards.map(card => (
          <div key={card.id} className="relative bg-white shadow-lg rounded-lg overflow-hidden">
            ...
            ></div>
            <button 
              onClick={() => handleLikeToggle(card.id)} // Add this onClick event
              className="absolute top-2 right-2 z-10 p-2 bg-white rounded-full shadow"
            >
              {likedCards.includes(card.id) ? ( // Change the heart icon based on whether the card is liked
                <svg
